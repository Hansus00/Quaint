...
\title{\Huge \textbf{Quaint}\\ \Large Interactive Wavepacket Simulator \\ User Manual}
\author{}
\date{}
...
\begin{enumerate}
    ...
    \item Press the \textbf{Play} button (or the Spacebar) to watch the wavepacket evolve in the potential well.
\end{enumerate}
...
% \begin{figure}[h]
%     \centering
%     % \includegraphics[width=0.9\textwidth]{images/animation_window.png}
%     \caption{Main 3D animation window showing the probability wave.}
% \end{figure}
...
\begin{itemize}
    \item \textbf{Play / Pause:} Toggles the animation playback. Alternatively, you can use the \textbf{Spacebar} as a global shortcut.
    \item \textbf{Timeline Slider:} Click and drag the slider to scrub through the animation manually.
    \item \textbf{Time Label:} Displays the current physical time elapsed in atomic units (a.u.) and the current frame index.
    \item \textbf{Reset Camera:} Instantly snaps the 3D camera back to its default viewing angle and distance.
    \item \textbf{Hide / Show Potential:} Toggles the visibility of the semi-transparent 3D potential mesh. Hiding it can give you a clearer, unobstructed view of the complex wave function.
    \item \textbf{Simulation Setup:} Opens the setup drawer, pausing the simulation and allowing you to redefine physical parameters and the environment.
    \item \textbf{Settings:} Opens a dedicated window for visual configuration (e.g., rendering scales, brightness).
...
% \begin{figure}[h]
%     \centering
%     % \includegraphics[width=0.9\textwidth]{images/setup_window.png}
...
You can modify the potential landscape using the \textbf{Editor Mode} tools:
\begin{itemize}
...
To gain finer control over the drawing tools, adjust the \textbf{Potential Editor Tools}:
\begin{itemize}
...
Instead of drawing by hand, you can automatically generate precise, mathematical potential landscapes using the \textbf{Load Preset Potential} dropdown:
\begin{itemize}
    \item \textbf{Custom / Clear:} Wipes the canvas clean to a flat, zero-potential state.
    \item \textbf{Gaussian Bump:} Generates a smooth, central potential hill. Useful for observing wave scattering and tunneling effects.
    \item \textbf{Harmonic Oscillator:} Creates a classic parabolic well originating from the center of the grid.
    ...
    \item \textbf{Matryoshka:} An embedded, nested potential setup featuring a central well surrounded by distinct barriers.
\end{itemize}

\subsection{Simulation Settings}
The right panel contains precise numerical parameters defining the physics engine behavior.

\subsubsection{Evolution \& Time}
\begin{itemize}
...
Achieved a fascinating quantum state? You can preserve your setup.
\begin{itemize}
...
\end{itemize}
Once you are satisfied with all parameters, click \textbf{Save \& Update Simulation} to build the matrices and begin the heavy calculation process.
...
\begin{itemize}
    ...
    \item \textbf{Wave Brightness Multiplier:} Increases the color exposure of the wave graph. This is incredibly useful for visualizing extremely faint probability tails that have tunneled through barriers.
